...
export async function POST(request: NextRequest) {
  try {
    ...
    const channel = await prisma.channel.findFirst({
      where: {
        ...
        OR: [
          ...
          { type: "TEAM" }, // Team channels accessible to all team members
          { isPrivate: false }, // Public channels accessible to all
    ...
    const membersToNotify = channel.members.filter(
      (member) => member.id !== user.id && member.email
    );
              ...
              html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                  <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                    <h2 style="color: #333; margin: 0;">New Message in ${
                      channel.name
                    }</h2>
                    ${
                      channel.project
                        ? `<p style="color: #666; margin: 5px 0 0 0;">Project: ${channel.project.name}</p>`
                        : ""
                    }
                  </div>
                  ...
                  <div style="background: white; padding: 20px; border: 1px solid #e9ecef; border-radius: 8px; margin-bottom: 20px;">
                    ...
                    ${
                      parentId
                        ? `<div style="margin-top: 10px; padding: 10px; background: #f8f9fa; border-left: 3px solid #007bff; font-size: 14px; color: #666;">Replying to a previous message</div>`
                        : ""
                    }
                  </div>
