...
export async function POST(request: NextRequest) {
  try {
    ...
    // Filter members based on their roles and access permissions
    const membersToNotify = channel.members.filter(member => 
      member.id !== user.id && 
      member.email &&
      // Only notify team members, admins, and project managers for project channels
      (channel.type !== 'PROJECT' || 
       ['TEAM_MEMBER', 'PROJECT_MANAGER', 'ADMIN', 'SUPER_ADMIN'].includes(member.role))
    )
              ...
              html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                  <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                    <h2 style="color: #333; margin: 0;">New Team Message in ${channel.name}</h2>
                    ${channel.project ? `<p style="color: #666; margin: 5px 0 0 0;">Project: ${channel.project.name}</p>` : ''}
                    ${channel.type === 'TEAM' ? '<p style="color: #666; margin: 5px 0 0 0;">Team Channel</p>' : ''}
                  </div>
                  ...
                  <div style="background: white; padding: 20px; border: 1px solid #e9ecef; border-radius: 8px; margin-bottom: 20px;">
                    <div style="display: flex; align-items: center; margin-bottom: 10px;">
                      ...
                      <span style="color: #666; margin-left: 8px; font-size: 12px;">${newMessage.sender.role}</span>
                    </div>
                  ...
                  <div style="text-align: center;">
                    <a href="${process.env.NEXTAUTH_URL}/team-member/chat" 
                  ...
                  <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #e9ecef; text-align: center; color: #666; font-size: 12px;">
                    <p>You're receiving this because you're a member of the ${channel.name} channel.</p>
